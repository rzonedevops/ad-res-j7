...
try:
    ...
    # Create placeholder classes for when framework is not available
...
FRAMEWORK_INSTANCES: Dict[str, Any] = {}
...
def validate_api_key(api_key):
    ...
    # If no keys configured, allow any key in development mode
    if not valid_keys and app.config.get('DEVELOPMENT_MODE', False):
        return len(api_key) >= 16  # Minimum key length
...
def require_auth(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not validate_api_key(api_key):
            logger.warning(f"Authentication failed for request to {request.endpoint}")
            return jsonify({
...
def create_case():
    ...
    try:
        ...
        """, (
            ...
            getattr(g, 'api_key', 'system')[:16],  # Use first 16 chars of API key as creator ID
...
if __name__ == "__main__":
    ...
    app.config['REQUIRE_AUTH'] = os.environ.get('REQUIRE_AUTH', 'True').lower() == 'true'
    app.config['DEVELOPMENT_MODE'] = os.environ.get('DEVELOPMENT_MODE', 'False').lower() == 'true'
    
    # Log authentication status
    auth_status = "enabled" if app.config['REQUIRE_AUTH'] else "disabled"
    dev_mode = "development" if app.config['DEVELOPMENT_MODE'] else "production"
    logger.info(f"Starting server with authentication {auth_status} in {dev_mode} mode")
    
    # Run server
    port = int(os.environ.get('FLASK_PORT', 5001))
    debug = os.environ.get('FLASK_DEBUG', 'False').lower() == 'true'
    app.run(debug=debug, port=port, host=os.environ.get('FLASK_HOST', '0.0.0.0'))