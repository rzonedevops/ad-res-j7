...
class AtomSpace:
    ...
    def __init__(self, case_id: str):
        """
        Initialize AtomSpace for a specific case.
        
        Args:
            case_id: Required unique identifier for the case. This parameter is mandatory
                    for SSR (Server-Side Rendering) and backend object construction.
        
        Raises:
            TypeError: If case_id is not provided
            ValueError: If case_id is None or empty string
        ...
        if case_id is None:
            raise ValueError(
                "AtomSpace.__init__() requires 'case_id' parameter. "
                "Please provide a valid case_id string. "
                "Example: AtomSpace(case_id='your_case_id_value')"
            )
        if not isinstance(case_id, str):
            raise TypeError(
                f"AtomSpace.__init__() case_id must be a string, got {type(case_id).__name__}. "
                "Example: AtomSpace(case_id='your_case_id_value')"
            )
        if not case_id.strip():
            raise ValueError(
                "AtomSpace.__init__() case_id cannot be empty. "
                "Please provide a valid case_id string. "
                "Example: AtomSpace(case_id='your_case_id_value')"
            )
