...
class HyperHolmesInferenceEngine:
    ...
    def run_abductive_inference(self) -> List[Dict[str, Any]]:
        """
        Run abductive inference to generate hypotheses from observed effects.
        Abductive reasoning: Given an observation, what is the most likely explanation?
        
        Returns:
            List of hypotheses generated through abductive reasoning
        """
        logger.info("Running abductive inference to generate hypotheses")
        
        hypotheses = []
        
        # Get all evidence atoms as observations
        evidence_atoms = self.atomspace.get_atoms_by_type(AtomType.EVIDENCE)
        
        if not evidence_atoms:
            logger.warning("No evidence atoms found for abductive inference")
            return hypotheses
        
        # Apply abductive rules to generate hypotheses
        for rule_id, rule in self.rules.items():
            if rule.rule_type == RuleType.ABDUCTION:
                try:
                    # Check if rule premises match observed evidence
                    matching_evidence = self._find_matching_evidence(rule, evidence_atoms)
                    
                    if matching_evidence:
                        hypothesis = self._generate_hypothesis_from_rule(rule, matching_evidence)
                        if hypothesis:
                            hypotheses.append(hypothesis)
                            logger.info(f"Generated hypothesis from rule {rule_id}: {hypothesis['name']}")
                
                except Exception as e:
                    logger.error(f"Error in abductive rule {rule_id}: {e}")
        
        # Score and rank hypotheses by explanatory power
        hypotheses = self._rank_hypotheses_by_explanatory_power(hypotheses, evidence_atoms)
        
        logger.info(f"Abductive inference complete: {len(hypotheses)} hypotheses generated")
        return hypotheses
    
    def _find_matching_evidence(self, rule: InferenceRule, evidence_atoms: List[Atom]) -> List[Atom]:
        """Find evidence atoms that match the rule's premises"""
        matching_evidence = []
        
        for premise in rule.premises:
            pattern = premise.get('pattern', '')
            atom_type = premise.get('atom_type')
            
            for evidence in evidence_atoms:
                # Check atom type match
                if atom_type and evidence.atom_type != atom_type:
                    continue
                
                # Check pattern match in name or metadata
                if pattern:
                    import re
                    text_to_search = f"{evidence.name} {evidence.metadata.get('source', '')} {evidence.metadata.get('category', '')}"
    ...
    def _generate_hypothesis_from_rule(self, rule: InferenceRule, evidence: List[Atom]) -> Optional[Dict[str, Any]]:
        """Generate a hypothesis from an abductive rule and matching evidence"""
        if not evidence:
            return None
        
        # Calculate confidence based on evidence strength and rule metadata
        confidence_scores = [e.truth_value.strength for e in evidence]
        average_evidence_strength = sum(confidence_scores) / len(confidence_scores)
        
        # Weight confidence by number of supporting evidence pieces
        evidence_count_bonus = min(0.2, len(evidence) * 0.05)  # Max 20% bonus
        total_confidence = min(0.95, average_evidence_strength + evidence_count_bonus)
        
        hypothesis = {
            'name': rule.conclusion.get('name', f'hypothesis_{rule.rule_id}'),
            'description': rule.conclusion.get('description', rule.description),
            'confidence_level': rule.conclusion.get('confidence_level', 'medium'),
            ...
            'evidence_details': [
                {
                    'id': e.atom_id,
                    'name': e.name,
                    'strength': e.truth_value.strength,
                    'source': e.metadata.get('source', 'unknown')
                } for e in evidence
            ],
            'explanatory_scope': len(evidence),
            'reasoning_type': 'abductive'
        }
        
        return hypothesis
    
    def _rank_hypotheses_by_explanatory_power(self, hypotheses: List[Dict[str, Any]], evidence_atoms: List[Atom]) -> List[Dict[str, Any]]:
        """Rank hypotheses by their explanatory power and evidence support"""
        
        for hypothesis in hypotheses:
            # Calculate explanatory power score
            evidence_explained = len(hypothesis.get('supporting_evidence', []))
            total_evidence = len(evidence_atoms)
            
            explanatory_coverage = evidence_explained / max(total_evidence, 1)
            confidence_score = hypothesis.get('confidence', 0)
            
            # Combined score: weighted average of coverage and confidence
            explanatory_power = (explanatory_coverage * 0.4) + (confidence_score * 0.6)
            hypothesis['explanatory_power'] = explanatory_power
        
        # Sort by explanatory power (highest first)
        hypotheses.sort(key=lambda h: h.get('explanatory_power', 0), reverse=True)
    ...
    def generate_hypotheses(self, evidence_atoms: List[str]) -> List[Dict[str, Any]]:
        ...
        # Use abductive inference to generate hypotheses
        abductive_hypotheses = self.run_abductive_inference()
        hypotheses.extend(abductive_hypotheses)
        
        # Add traditional hypothesis generation as backup
        if not hypotheses:
