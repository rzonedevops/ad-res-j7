...
class ReZonanceCaseAnalyzer:
    ...
    def load_entities(self) -> List[ReZonanceEntity]:
        ...
        entities_data = [
            ...
            },
            {
                "name": "Joziway",
                "entity_type": "Company",
                "description": "Related entity mentioned in correspondence",
                "contact_info": {"registration": "Unknown"},
    ...
    def load_timeline_events(self) -> List[ReZonanceTimelineEvent]:
        ...
        events_data = [
            ...
            },
            {
                ...
                "event_type": "False Payment Claim",
                "description": "RegimA claims payment of R470,000 not reflected in ReZonance records",
                "entity_involved": "Rynette Farrar",
                "financial_amount": Decimal("470000.00"),
                "document_reference": "RegimA Accounting Records",
                "significance_level": 5
            },
            {
                "event_date": datetime.date(2023, 9, 20),
                "event_type": "Additional False Claims",
                "description": "Additional false payment claims totaling R765,361.34",
                "entity_involved": "Rynette Farrar",
                "financial_amount": Decimal("765361.34"),
                "document_reference": "RegimA Accounting Records",
                "significance_level": 5
            },
            {
            ...
            {
                "event_date": datetime.date(2025, 10, 9),
                "event_type": "Fraud Discovery",
                "description": "Payment fraud scheme discovered through financial analysis",
                "entity_involved": "Rynette Farrar",
                "financial_amount": Decimal("1235361.34"),
                "document_reference": "Forensic Analysis Report",
                "significance_level": 5
            }
        ]
    ...
    def analyze_payment_fraud(self) -> Dict[str, Any]:
        """Analyze payment fraud in the ReZonance case."""
        if not self.fraud_analyzer:
            self.fraud_analyzer = create_rezonance_fraud_analyzer()
            
        # Generate comprehensive fraud analysis
        fraud_analysis = self.fraud_analyzer.export_fraud_analysis()
    ...
    def generate_hypergraph_nodes(self) -> List[Dict[str, Any]]:
        ...
        # Fraud pattern nodes
        if self.fraud_analyzer:
            for i, pattern in enumerate(self.fraud_analyzer.detected_patterns):
                nodes.append({
                    "id": f"fraud_pattern_{i+1}",
                    "type": "fraud_pattern",
    ...
    def generate_hypergraph_edges(self) -> List[Dict[str, Any]]:
        ...
        # Fraud relationship edges
        edges.append({
            "id": "fraud_perpetrator",
            "type": "criminal",
            "source": "entity_rynette_farrar",
            "target": "entity_regima_skin_treatments_cc",
            "relationship": "fraud_perpetrator",
            "status": "active"
        })
        
        # Connect fraud patterns to entities if fraud analyzer exists
        if self.fraud_analyzer:
            for i, pattern in enumerate(self.fraud_analyzer.detected_patterns):
                edges.append({
                    "id": f"fraud_pattern_{i+1}_connection",
                    "type": "evidence",
                    "source": f"fraud_pattern_{i+1}",
                    "target": "entity_rynette_farrar",
    ...
    def export_to_json(self, filename: str = "rezonance_case_data.json") -> str:
        ...
        # Ensure fraud analysis is complete
        if not self.fraud_analyzer:
            self.analyze_payment_fraud()
        ...
        case_data = {
            ...
            "fraud_analysis": self.fraud_analyzer.export_fraud_analysis() if self.fraud_analyzer else {},
...
def main():
    ...
    print("Analyzing payment fraud...")
    fraud_analysis = analyzer.analyze_payment_fraud()
    print(f"Fraud amount: R{fraud_analysis['fraud_pattern_analysis']['fraud_scheme']['total_fraudulent_amount']:,.2f}")
    print(f"Fraud classification: {fraud_analysis['case_classification']}")
