...
class TestAtomSpace:
    ...
    def test_atomspace_requires_case_id(self):
        """Test that AtomSpace raises TypeError when case_id is not provided"""
        import pytest
        with pytest.raises(TypeError, match="missing 1 required positional argument"):
            AtomSpace()  # Should raise TypeError
    
    def test_atomspace_rejects_none_case_id(self):
        """Test that AtomSpace raises ValueError when case_id is None"""
        import pytest
        with pytest.raises(ValueError, match="requires 'case_id' parameter"):
            AtomSpace(case_id=None)
    
    def test_atomspace_rejects_empty_case_id(self):
        """Test that AtomSpace raises ValueError when case_id is empty"""
        import pytest
        with pytest.raises(ValueError, match="case_id cannot be empty"):
            AtomSpace(case_id="")
        with pytest.raises(ValueError, match="case_id cannot be empty"):
            AtomSpace(case_id="   ")  # Only whitespace
    
    def test_atomspace_rejects_non_string_case_id(self):
        """Test that AtomSpace raises TypeError when case_id is not a string"""
        import pytest
        with pytest.raises(TypeError, match="case_id must be a string"):
            AtomSpace(case_id=123)
        with pytest.raises(TypeError, match="case_id must be a string"):
            AtomSpace(case_id=["case_id"])
        with pytest.raises(TypeError, match="case_id must be a string"):
            AtomSpace(case_id={"case": "id"})
