...
class Config:
    ...
    def validate(self, strict: bool = False) -> bool:
        ...
        Args:
            strict: If True, require both Supabase and Neon credentials.
                   If False, require at least one database connection.
        ...
        # Check Neon credentials (either connection string or individual components)
        has_neon = bool(
            self.database.neon_connection_string or (
                self.database.neon_host and 
                self.database.neon_database and 
                self.database.neon_user and 
                self.database.neon_password
            )
        )
        
        if strict:
            # Strict mode: require both databases
            missing_fields = []
            if not has_supabase:
                missing_fields.extend(["SUPABASE_URL", "SUPABASE_KEY"])
            if not has_neon:
                missing_fields.append("NEON_CONNECTION_STRING or (NEON_HOST, NEON_DATABASE, NEON_USER, NEON_PASSWORD)")
            
            if missing_fields:
                print(f"Missing required configuration: {', '.join(missing_fields)}")
                return False
        else:
            # Non-strict mode: require at least one database
            if not has_supabase and not has_neon:
                print("Missing database configuration: At least one of Supabase or Neon must be configured")
                print("  Supabase: Set SUPABASE_URL and SUPABASE_KEY")
                print("  Neon: Set NEON_CONNECTION_STRING or (NEON_HOST, NEON_DATABASE, NEON_USER, NEON_PASSWORD)")
                return False
