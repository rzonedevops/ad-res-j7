...
COMMENT ON TABLE affidavit_sections IS 'Sections of Peter''s Founding Affidavit (e.g., Section 7: Discovery of Financial Irregularities)';
...
COMMENT ON TABLE ad_paragraphs IS 'All 141 AD paragraphs from Peter''s Founding Affidavit';
COMMENT ON COLUMN ad_paragraphs.paragraph_number IS 'AD paragraph number (e.g., 7.1, 10.9.3.1)';
COMMENT ON COLUMN ad_paragraphs.severity IS '1=Low priority, 5=Critical/urgent';
...
COMMENT ON TABLE jr_responses IS 'Jacqueline''s responses (JR paragraphs) to AD paragraphs';
COMMENT ON COLUMN jr_responses.paragraph_number IS 'JR paragraph number (e.g., 7.1, 7.5.1)';
COMMENT ON COLUMN jr_responses.ad_paragraph_ref IS 'AD paragraph being responded to';
COMMENT ON COLUMN jr_responses.evidence_strength IS '1=Weak evidence, 5=Strong documentary evidence';
...
COMMENT ON TABLE dr_responses IS 'Daniel''s responses (DR paragraphs) to AD paragraphs';
COMMENT ON COLUMN dr_responses.paragraph_number IS 'DR paragraph number (e.g., 7.1, 7.5.1)';
COMMENT ON COLUMN dr_responses.ad_paragraph_ref IS 'AD paragraph being responded to';
COMMENT ON COLUMN dr_responses.evidence_strength IS '1=Weak evidence, 5=Strong documentary evidence';
...
CREATE TABLE IF NOT EXISTS comments (
    ...
    comment_type TEXT CHECK (comment_type IN ('strategic', 'legal', 'evidence', 'contradiction', 'perjury', 'material_non_disclosure')),
    content TEXT NOT NULL,
...
COMMENT ON TABLE comments IS 'Strategic notes, legal arguments, and commentary on AD paragraphs';
COMMENT ON COLUMN comments.comment_type IS 'Type of commentary (strategic, legal, evidence, contradiction, perjury, material_non_disclosure)';
...
CREATE TABLE IF NOT EXISTS annexures (
    ...
    annexure_type TEXT CHECK (annexure_type IN ('PF', 'JF', 'DF', 'SF')),  -- Peter/Jacqueline/Daniel/Supporting
    referenced_by TEXT[],  -- Array of paragraph numbers that reference this annexure
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

COMMENT ON TABLE annexures IS 'Evidence annexures referenced in affidavits';
COMMENT ON COLUMN annexures.annexure_code IS 'Unique annexure identifier (e.g., PF6, JF-VV1, DF4)';
COMMENT ON COLUMN annexures.annexure_type IS 'PF=Peter, JF=Jacqueline, DF=Daniel, SF=Supporting';
...
CREATE TABLE IF NOT EXISTS timeline_events (
    ...
    event_category TEXT CHECK (event_category IN ('financial', 'operational', 'legal', 'communication', 'fraud', 'retaliation')),
    significance INTEGER CHECK (significance BETWEEN 1 AND 5),  -- 1=Low, 5=Critical
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);

COMMENT ON TABLE timeline_events IS 'Key events referenced in affidavits for temporal analysis';
COMMENT ON COLUMN timeline_events.significance IS '1=Minor event, 5=Critical turning point';
...
COMMENT ON TABLE legal_principles IS 'Legal principles from lex/* framework mapped to AD paragraphs';
COMMENT ON COLUMN legal_principles.confidence IS 'Confidence score (0.00 to 1.00) for principle applicability';
...
SELECT 
    ...
    ad.severity,
    CASE WHEN jr.paragraph_number IS NOT NULL THEN 'Yes' ELSE 'No' END AS has_jr_response,
    jr.paragraph_number AS jr_number,
    CASE WHEN dr.paragraph_number IS NOT NULL THEN 'Yes' ELSE 'No' END AS has_dr_response,
...
COMMENT ON VIEW ad_coverage_summary IS 'Summary of JR/DR response coverage for all AD paragraphs';
...
SELECT 
    ...
    ad.severity,
    CASE 
        WHEN jr.paragraph_number IS NULL AND dr.paragraph_number IS NULL THEN 'Both'
        WHEN jr.paragraph_number IS NULL THEN 'JR only'
        WHEN dr.paragraph_number IS NULL THEN 'DR only'
...
COMMENT ON VIEW missing_responses IS 'AD paragraphs that need JR or DR responses';
