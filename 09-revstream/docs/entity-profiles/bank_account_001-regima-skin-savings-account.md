# Entity Profile: Regima Skin Savings Account

---

## Entity Information

| Field | Value |
|-------|-------|
| **Entity ID** | BANK_ACCOUNT_001 |
| **Entity Name** | Regima Skin Savings Account |
| **Entity Type** | Bank Account |
| **Category** | bank_accounts |
| **Account Number** | 62134839127 |
| **Account Type** | Money Market Transactor |

---

## Summary Statistics

| Metric | Value |
|--------|-------|
| **Events Involved** | 1 |
| **Outgoing Relations** | 0 |
| **Incoming Relations** | 1 |
| **Total Financial Impact** | R5,460,000.00 |

---

## Primary Actions

*No primary actions documented*

---

## Events Timeline

This entity is involved in **1 events** across the case timeline.

### Phase 6: Cover-up Phase

#### EVENT_070: Systematic Account Emptying

**Date:** 2025-09-11  
**Financial Impact:** R5,460,000.00  

R5.46M transferred from all operating accounts to savings accounts, leaving minimal balances insufficient for business operations. This represents 99.3% of operating funds removed.

**Evidence:**
- `CCE20250911_0000.jpg`
- `CCE20250911_0001.jpg`
- `CCE20250911_0002.jpg`
- *...and 5 more*

---

## Relationships

### Outgoing Relations

*No outgoing relations documented.*

### Incoming Relations (1)

Relations where this entity is the **target**:

#### CONTROLS

- **PERSON_001 →**: Peter Faucitt controls Regima Skin Savings Account, used in systematic emptying

---

## Financial Impact Analysis

### Impact by Category

| Category | Amount |
|----------|--------|
| Financial Strangulation | R5,460,000.00 |
| **TOTAL** | **R5,460,000.00** |

---

## Evidence References

This entity is referenced in **8 evidence files**:

- `CCE20250911_0000.jpg`
- `CCE20250911_0001.jpg`
- `CCE20250911_0002.jpg`
- `CCE20250911_0003.jpg`
- `CCE20250911_0004.jpg`
- `CCE20250911_0005.jpg`
- `CCE20250911_0006.jpg`
- `CCE20250911_0007.jpg`

---

## GitHub Pages References

- [Entity Reference](https://cogpy.github.io/revstream1/application-2-evidence.md#bank-accounts)

**Event References:**
- [application-2-evidence.md#account-emptying](https://cogpy.github.io/revstream1/application-2-evidence.md#account-emptying)

---

## Extended Evidence Repository

For additional evidence and supporting documentation, see the [ad-res-j7 repository](https://github.com/cogpy/ad-res-j7).

**Direct Reference:** `new_evidence_2025_11_25/images/CCE20250911_0000.jpg`

---

*Entity profile generated: 2025-11-25 07:09:36*  
*Data models version: Entities v19.0, Events v20.0, Relations v16.0*
