...
TIMELINE_FILE = f"{DATA_MODELS_DIR}/timelines/timeline.json"
RELATIONS_FILE = f"{DATA_MODELS_DIR}/relations/relations.json"
MAPPING_FILE = f"{BASE_DIR}/ad-res-j7/EVIDENCE_MAPPING.json"
...
def generate_entity_pages(entities_data):
    ...
    all_entities = []
    for category in entities_data['entities']:
        all_entities.extend(entities_data['entities'][category])
...
def generate_event_pages(events_data, evidence_mapping):
    ...
    for event in events_data['events']:
        ...
        if evidence_mapping:
            ...
            content += "No evidence mapping file found.\n"
...
def generate_timeline_page(timeline_data):
    ...
    for event in sorted(timeline_data['timeline'], key=lambda x: x.get('date', '')):
        event_id = event.get('event_id')
    ...
    print(f"  Generated timeline page with {len(timeline_data['timeline'])} events.")

def generate_index_page(entities, events, relations, timeline):
...
| **Entities Profiled** | {total_entities} |
...
### 📋 Core Documentation
- **[Interactive Timeline](timeline.md)** - A complete, chronological timeline of all {total_events} events with evidence links.
- **[Entity Profiles](entities/)** - Detailed profiles of all {total_entities} key persons and organizations.
- **[Event Details](events/)** - Individual pages for each of the {total_events} timeline events.
...
def main():
    ...
    generate_index_page(entities, events, relations, timeline)
