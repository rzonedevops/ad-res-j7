...
def analyze_relations(relations_data):
    ...
    issues = []
    stats = {
        'total': len(relations_data['relations']),
        'missing_evidence': 0,
        'missing_events': 0,
    ...
    for relation in relations_data['relations']:
        if not relation.get('evidence') or len(relation.get('evidence', [])) == 0:
            stats['missing_evidence'] += 1
            issues.append(f"RELATION {relation['relation_id']}: No evidence references")
        
        if not relation.get('related_events') or len(relation.get('related_events', [])) == 0:
            stats['missing_events'] += 1
            issues.append(f"RELATION {relation['relation_id']}: No events linked")
